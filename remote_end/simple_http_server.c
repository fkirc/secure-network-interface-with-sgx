...
static const char EXPECTED_REQUEST[] = "GET / HTTP/1.1\r\n\r\n";
...
static void run_server() {
    ...
    // Set SO_LINGER with zero time-out to avoid TIME_WAIT state for a large number of connections
    struct linger l = {0};
    l.l_onoff = 1;
    l.l_linger = 0; // timeout in seconds
    if (setsockopt(listen_fd, SOL_SOCKET, SO_LINGER, (const char *) &l, sizeof(l))) {
...
static void serve_respond(int client_fd) {
    ...
    char buf[MSG_SIZE(EXPECTED_REQUEST)];

    int n_rec = read_n(client_fd, buf, MSG_SIZE(EXPECTED_REQUEST));
    assert(n_rec == (int) MSG_SIZE(EXPECTED_REQUEST));

    assert(!memcmp(buf, EXPECTED_REQUEST, MSG_SIZE(EXPECTED_REQUEST)));

    int n_sent = write_n(client_fd, HTTP_RESPONSE, MSG_SIZE(HTTP_RESPONSE));
    assert(n_sent == MSG_SIZE(HTTP_RESPONSE));
